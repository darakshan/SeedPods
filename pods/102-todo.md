...
- **Pending-changes badge and AI commit message** — Show the number of uncommitted changes in the app nav bar, visible across all modes (chat, browse, split, feedback, settings). Invisible at zero. Also set the iOS app icon badge to the same count — visible on the home screen without opening the app. Tapping the badge asks "Commit N changes?" On yes, the server passes the list of changed files to the AI, which writes a one-line commit message. If "require edit approval" is on, the user sees and approves the message like any edit. If approval is off, it commits automatically. Same machinery as existing edits — no new UI patterns, no background processes. Just the badge, a confirmation, and the edit channel.
