#!/usr/bin/env python3
"""
check.py — Review seedpods for structure guidelines.
- Surface/depth word counts vs config/settings.txt limits
- Every seedpod pointed to by at least min_related_in_degree others (#related)
- Underlinked: seedpods with 0 #related
- #related max 5
- Report @note directives (editorial comments; removed from content, printed at build time)
- Status vs sections (surface, depth, script, images; provenance ignored): section counts and valid statuses from config/status.txt; word count for statuses from top of config up through last draft*

Usage:
  python src/check.py              # summary + detailed findings (all seedpods)
  python src/check.py -q            # summary only
  python src/check.py -v            # summary + details + notes (interleaved by seedpod)
  python src/check.py 3             # check only 003, show notes (implies -v)
  python src/check.py 3 4 19 99     # check 003, 004, 019; warn 99 missing
"""

import sys

from category_colors import load_category_colors
from reporter import error as reporter_error, has_errors, note as reporter_note, print_all, reset as reporter_reset, warning as reporter_warning
from seedpod_parser import (
    load_all_seedpods,
    load_index_copy,
    load_status_order,
    section_is_tbd,
)

PRIMARY_CATEGORIES = frozenset(load_category_colors().keys())

REQUIRED_STATUS_NAMES = frozenset({"empty", "prelim", "proto", "partial", "draft1", "rough"})
COMPLETE_STATUS_NAMES = frozenset({"draft1", "rough"})
ONE_SECTION_STATUS_NAMES = frozenset({"prelim", "proto"})


def _word_count(text):
    if not text or section_is_tbd(text):
        return 0
    return len(text.split())


def load_index_params():
    out = {
        "surface_min_words": 400,
        "surface_max_words": 700,
        "depth_min_words": 300,
        "depth_max_words": 600,
        "min_related_in_degree": 2,
    }
    copy = load_index_copy()
    for key in ("surface_min_words", "surface_max_words", "depth_min_words", "depth_max_words", "min_related_in_degree"):
        if key in copy:
            try:
                out[key] = int(copy[key])
            except ValueError:
                pass
    return out


def _validate_status_order(status_order):
    """Ensure status_order contains required names. Returns True if valid. Errors once if not."""
    status_set = set(status_order or [])
    missing = REQUIRED_STATUS_NAMES - status_set
    if missing:
        reporter_error(
            "config/status.txt must define these status names: {}".format(", ".join(sorted(missing)))
        )
        return False
    return True


def _complete_statuses(status_order):
    """Statuses that mean 'has all 4 sections': draft1, rough (by name)."""
    if not status_order:
        return set()
    return set(status_order) & COMPLETE_STATUS_NAMES


def _run_word_count_statuses(status_order):
    """Statuses that get surface/depth word-count checks: from top of config up through last draft*."""
    if not status_order:
        return set()
    last_draft_i = -1
    for i, s in enumerate(status_order):
        if s.startswith("draft"):
            last_draft_i = i
    if last_draft_i < 0:
        return set()
    return set(status_order[: last_draft_i + 1])


def _run_other_limits(status, status_order):
    """True if seedpod gets in-degree and #related checks (complete but maybe rough)."""
    complete = _complete_statuses(status_order)
    return status in complete


def main():
    verbose = "-v" in sys.argv or "--verbose" in sys.argv
    quiet = "-q" in sys.argv or "--quiet" in sys.argv
    args = [a for a in sys.argv[1:] if a not in ("-v", "--verbose", "-q", "--quiet")]
    seedpod_filter = None
    if args and all(a.isdigit() for a in args):
        seedpod_filter = set(a.zfill(3) for a in args)
        verbose = True

    reporter_reset()
    params = load_index_params()
    seedpods = load_all_seedpods(warn=lambda msg, filepath=None: None)
    status_order = load_status_order()
    status_order_valid = _validate_status_order(status_order)
    all_numbers = set(n.get("number", "") for n in seedpods if n.get("number"))
    if seedpod_filter:
        for a in args:
            n = a.zfill(3)
            if n not in all_numbers:
                reporter_error("no such seedpod", seedpod_num=n)
        seedpods_to_check = [n for n in seedpods if n.get("number") in seedpod_filter]
    else:
        seedpods_to_check = seedpods

    if status_order_valid:
        for n in seedpods_to_check:
            status = n.get("status", "")
            if status and status not in REQUIRED_STATUS_NAMES:
                num = n.get("number", "?")
                fn = n.get("filename", "?")
                shortname = fn.split("-", 1)[-1] if "-" in fn else None
                reporter_error("check does not recognize status {!r}".format(status), seedpod_num=num, shortname=shortname)
                print_all()
                sys.exit(1)

    errors = []
    in_degree = {}
    for n in seedpods:
        num = n.get("number", "")
        if num:
            in_degree[num] = in_degree.get(num, 0)
        for r in n.get("related", []):
            in_degree[r] = in_degree.get(r, 0) + 1

    for n in seedpods_to_check:
        num = n.get("number", "?")
        fn = n.get("filename", "?")
        layers = n.get("layers", {})
        status = n.get("status", "")

        surface = layers.get("surface", "TBD")
        depth = layers.get("depth", "TBD")
        s_words = _word_count(surface)
        d_words = _word_count(depth)
        run_word_count = status in _run_word_count_statuses(status_order)
        run_limits = _run_other_limits(status, status_order)
        shortname = fn.split("-", 1)[-1] if "-" in fn else None

        if run_word_count and not section_is_tbd(surface):
            lo, hi = params["surface_min_words"], params["surface_max_words"]
            if s_words < lo:
                msg = "surface has {} words (min {})".format(s_words, lo)
                errors.append(("length", msg))
                reporter_error(msg, seedpod_num=num, shortname=shortname)
            elif s_words > hi:
                msg = "surface has {} words (max {})".format(s_words, hi)
                errors.append(("length", msg))
                reporter_error(msg, seedpod_num=num, shortname=shortname)

        if run_word_count and not section_is_tbd(depth):
            lo, hi = params["depth_min_words"], params["depth_max_words"]
            if d_words < lo:
                msg = "depth has {} words (min {})".format(d_words, lo)
                errors.append(("length", msg))
                reporter_error(msg, seedpod_num=num, shortname=shortname)
            elif d_words > hi:
                msg = "depth has {} words (max {})".format(d_words, hi)
                errors.append(("length", msg))
                reporter_error(msg, seedpod_num=num, shortname=shortname)

        if run_limits:
            degree = in_degree.get(num, 0)
            min_deg = params["min_related_in_degree"]
            if degree < min_deg:
                msg = "in-degree {} (min {} via #related)".format(degree, min_deg)
                errors.append(("in_degree", msg))
                reporter_error(msg, seedpod_num=num, shortname=shortname)

        related = n.get("related", [])
        if run_limits and len(related) == 0:
            msg = "0 #related"
            errors.append(("underlinked", msg))
            reporter_error(msg, seedpod_num=num, shortname=shortname)
        elif run_limits and len(related) > 5:
            msg = "#related has {} entries (max 5)".format(len(related))
            errors.append(("over_related", msg))
            reporter_error(msg, seedpod_num=num, shortname=shortname)

        has_unheaded_text = not section_is_tbd(layers.get("brief"))
        if has_unheaded_text and status != "proto":
            msg = "text without a section header"
            errors.append(("structure", msg))
            reporter_error(msg, seedpod_num=num, shortname=shortname)

        section_names = ("surface", "depth", "script", "images")
        n_sections = sum(1 for name in section_names if not section_is_tbd(layers.get(name)))
        if status_order_valid:
            complete_statuses = _complete_statuses(status_order)
            if n_sections == 0:
                if status not in ("empty", "proto"):
                    msg = "has {} section(s) but status is {!r} (expected empty or proto)".format(n_sections, status)
                    errors.append(("status", msg))
                    reporter_error(msg, seedpod_num=num, shortname=shortname)
            elif n_sections == 1:
                if status not in ONE_SECTION_STATUS_NAMES:
                    msg = "has 1 section but status is {!r} (expected one of: {})".format(status, ", ".join(sorted(ONE_SECTION_STATUS_NAMES)))
                    errors.append(("status", msg))
                    reporter_error(msg, seedpod_num=num, shortname=shortname)
            elif n_sections >= 2 and n_sections <= 3:
                expected_status = "partial"
                if status != expected_status:
                    msg = "has {} section(s) but status is {!r} (expected {})".format(n_sections, status, expected_status)
                    errors.append(("status", msg))
                    reporter_error(msg, seedpod_num=num, shortname=shortname)
            else:
                if status not in complete_statuses:
                    expected_str = ", ".join(sorted(complete_statuses))
                    msg = "has all 4 sections but status is {!r} (expected one of: {})".format(status, expected_str)
                    errors.append(("status", msg))
                    reporter_error(msg, seedpod_num=num, shortname=shortname)

        category = n.get("category", "")
        if not category:
            msg = "no #category (must be one of the primary categories)"
            errors.append(("primary_category", msg))
            reporter_error(msg, seedpod_num=num, shortname=shortname)
        elif category not in PRIMARY_CATEGORIES:
            msg = "#category {!r} is not a primary category (expected one of: {})".format(
                category, ", ".join(sorted(PRIMARY_CATEGORIES))
            )
            errors.append(("primary_category", msg))
            reporter_error(msg, seedpod_num=num, shortname=shortname)

    counts = {}
    for kind, _ in errors:
        counts[kind] = counts.get(kind, 0) + 1
    n_seedpods = len(seedpods_to_check)
    n_issues = len(errors)
    n_notes = sum(len(n.get("notes", [])) for n in seedpods_to_check)

    parts = [f"{n_seedpods} seedpods", f"{n_notes} notes"]
    if counts.get("length"):
        parts.append(f"{counts['length']} length")
    if counts.get("in_degree"):
        parts.append(f"{counts['in_degree']} in-degree")
    if counts.get("underlinked"):
        parts.append(f"{counts['underlinked']} underlinked")
    if counts.get("over_related"):
        parts.append(f"{counts['over_related']} over-related")
    if counts.get("status"):
        parts.append(f"{counts['status']} status")
    if counts.get("primary_category"):
        parts.append(f"{counts['primary_category']} primary_category")
    suffix = f" — {n_issues} issues" if n_issues else " — ok"
    summary = ", ".join(parts) + suffix + "."
    print(summary, file=sys.stderr)

    by_status = {}
    for n in seedpods_to_check:
        s = n.get("status", "empty")
        by_status[s] = by_status.get(s, 0) + 1
    ordered = [s for s in status_order if s in by_status]
    ordered += [s for s in sorted(by_status) if s not in status_order]
    status_line = ", ".join(f"{s}: {by_status[s]}" for s in ordered)
    print(status_line, file=sys.stderr)

    if verbose:
        for n in seedpods_to_check:
            if n.get("notes"):
                num = n.get("number", "?")
                fn = n.get("filename", "?")
                shortname = fn.split("-", 1)[-1] if "-" in fn else None
                for note in n.get("notes", []):
                    reporter_note(note, seedpod_num=num, shortname=shortname)

    if not quiet:
        print_all()

    if has_errors():
        sys.exit(1)
    sys.exit(0)


if __name__ == "__main__":
    main()
