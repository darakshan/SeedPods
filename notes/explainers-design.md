...
**Where links live:** In the nugget where the term is introduced. Same place as the `#term` line: inside `#provenance`. So the canonical store is the nugget file.

**How (when we implement):** Add an optional explainer URL per term (and duration for display). For example a directive immediately after `#term` in provenance, e.g. `#explainer https://... (13 min)`, applying to the preceding term. Parser would expose terms as (term, definition, url_or_empty, duration_or_empty). Glossary page (and any other place we present terms) shows the link when present, with the duration next to it — e.g. "Explain (13 min)" or "Explain (45 min)". No need to label long ones "deep dive"; the length is enough.
