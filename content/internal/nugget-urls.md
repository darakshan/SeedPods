# Finding a pod page on the website

The list of all pods and their revision numbers is available at **./list.html** (relative to the site root).

The build gets the site root URL and output path from **config/settings.txt** (`site_base` and `site_dir`). Given the site root URL, a pod number, and its shortname, you can form the canonical URL for that pod’s page.
...
- **number** — the pod’s numeric id, zero-padded to 3 digits (e.g. `001`, `072`).
- **shortname** — the slug after the number in the pod filename (e.g. `caloric`, `temperature`).
