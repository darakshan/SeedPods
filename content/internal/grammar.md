...
The build script parses pod files strictly. Follow this grammar so new or revised pods work without manual processing.
...
- Required fields and format:
  ...
  - `#related` — comma-separated list of other pod numbers. The NNN from each target's filename (e.g. 002, 011, 018). Max 5. Links resolve by string equality, so "1" will not match a pod whose filename starts with "001".
...
- The build does two things with `#ref` lines: (1) At the end of the References layer on each pod page it generates a **Further reading** subsection listing all refs from that pod. (2) It collects refs from **all** pods and generates the Bibliography page, sorted by exact ref text (so author order if you cite as "Author, Title..."), with which pods cite each.

###Key terms (#term)

- **Anywhere in a pod.** Each line: `#term` followed by a space and the term, then a colon (`:`), then the definition (e.g. `#term Paradigm: Thomas Kuhn's term for the framework...`). One term per line.
- Use `#term` for any concept or phrase the pod introduces, references, or relies on — including what used to be additional tags. If no definition is ready yet, use `TBD` or write a specific open question (e.g. `#term autopoiesis: TBD — does this apply outside biological cells?`).
- The build collects terms from **all** pods and generates the Glossary page: terms sorted alphabetically, grouped so the same term from multiple pods appears once with each definition indented; each entry lists which pods define it (In: …).
...
- All five layer headers must appear in this order: `#surface`, `#depth`, `#provenance`, `#script`, `#images`. If a layer has no content yet, write the header and put `TBD` (or a single line of placeholder text) as the body so the section exists. Exception: pods with `#status proto` have **no** primary section headers (`#brief`, `#surface`, `#depth`, `#script`, `#images`); they have an unheaded proto body and may optionally have `#provenance` (and `#term`, `#ref`).
...
- Related: meta["related"] is split on commas, each item stripped. Matching to other pods is by exact string equality of the pod number (the NNN from the filename).

###Minimal valid pod (template)
...
Every pod must have **exactly one** primary category in `#category`. The seven primary categories are:

| Category   | Use when the pod's main hook is… |
|------------|-------------------------------------|
...
- **Rule**: `#category` must be exactly one of these seven (exact spelling, including `AI-minds`). All other concepts the pod touches are represented as `#term` entries.
- **Rule of thumb**: If the pod's hook is sensation (e.g. color, consonance), use **sensation** even when the explanation leans on mathematics or physics.
- The home page and categories page group pods by `#category`. The check tool (`just check`) fails if a pod has no `#category` or if the value is not in this set.
