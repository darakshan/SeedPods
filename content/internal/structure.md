...
Every seed pod has the same five layers. This consistency makes the archive navigable and the template maintainable. A reader who knows the structure can find what they need at whatever depth they want.

### Proto body (no primary sections)

Pods with status **proto** have an unheaded body (the proto text) and no primary section headers. Do **not** use `#brief`, `#surface`, `#depth`, `#script`, or `#images` in proto pods; the build derives the “brief” content from the first block of text after the metadata. Proto pods may optionally include **#provenance** (with prose, `#term`, and `#ref`). These are created by the import command from prototype .md files (e.g. in `content/more/`). Each protopod in the **import source** (.md) must include a `#shortname` line so the importer can assign the output filename; import skips any block that lacks it. The resulting pod .txt file does not contain `#number` or `#shortname` — those are derived from the filename only. Proto pods may later be promoted to full pods by adding the five layers and changing status.
...
- number and short name (from filename: NNN-shortname.txt; do not put `#number` or `#shortname` in the pod file)
...
- references (#ref): optional lines inside **#provenance** only; each line is full citation text. The build generates a “Further reading” subsection on the pod page and a shared Bibliography page (sorted by ref text).
- key terms (#term): optional lines anywhere in a pod; each line is `#term Term: Definition`. Use for any concept the pod introduces or relies on — this is how all non-category subject matter is tracked. The build generates the shared Glossary page (terms grouped, definitions indented; lists which pods define each).
...
Use **rough** when the pod has all sections but they need substantial revision. Use **draft1** when the pod is complete but still open to revision, **final** when it is locked. The check tool reports a mismatch if status does not match the section count.

Pods with status **proto** have an unheaded body (rendered as “Brief”) and optionally **#provenance**; they do not use the standard five primary layers. The site does not show the full layer-tabs nav for proto pods.